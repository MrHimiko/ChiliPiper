...
jQuery(document).ready(function ($) {
  ...
  $(window).on("load", function () {
    ...
    if (searchParams.get('param0')) {
      ...
      setTimeout(function() { $('.section--rio__form .button').trigger("click"); }, 1500);
  ...
  function drawTrendlines(perc1, perc2, container) {
    var data = google.visualization.arrayToDataTable([
      ['Before Chili Piper', 'Percentage', { role: 'style' }],
    ...
    var options = {
      ...
      trendlines: {
        0: {type: 'linear', lineWidth: 1, opacity: .3},
        1: {type: 'exponential', lineWidth: 1, opacity: .3}
      },
