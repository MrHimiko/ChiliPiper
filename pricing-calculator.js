...
var TRIGGER_SELECTOR = '[data-offcanvas-trigger]';
var OFFCANVAS_SELECTOR = '[data-offcanvas]';

var offcanvas = function () {
  var triggers = document.querySelectorAll(TRIGGER_SELECTOR);
  var offcanvas = document.querySelector(OFFCANVAS_SELECTOR);
  var media = window.matchMedia('(min-width: 980px)');

  if (triggers.length && offcanvas) {
    for (var i = 0; i < triggers.length; i++) {
      triggers[i].addEventListener('click', function (e) {
        e.preventDefault();
        offcanvas.classList.toggle(offcanvas.dataset.activeClass);
      });
    }
  }
  ...
  media.addListener(function (media) {
    if (!media.matches) {
      offcanvas.classList.remove(offcanvas.dataset.activeClass);
    }
  });
};
...
var navbar = function () {
  var scroll = debounce(function () {
    ...
    if (scrollTop > 80) {
      navbar.classList.add(navbar.dataset.activeClass);
    } else {
      navbar.classList.remove(navbar.dataset.activeClass);
    }
  }, 50);
...
var TRIGGER_SELECTOR$1 = '[data-accordion-label]';

var accordion = function () {
  var accordions = document.querySelectorAll(ACCORDION_SELECTOR);

  for (var i = 0; i < accordions.length; i++) {
    var trigger = accordions[i].querySelector(TRIGGER_SELECTOR$1);

    trigger.addEventListener('click', function () {
      this.parentElement.classList.toggle(this.parentElement.dataset.activeClass);
...
function onTouchStart(event) {
  ...
  if (e.type !== 'touchstart') {
    ...
    if (doc$1.activeElement && $(doc$1.activeElement).is(data$$1.formElements) && doc$1.activeElement !== e.target) {
...
var Virtual = {
  ...
  renderSlide: function renderSlide(slide, index$$1) {
    ...
    var $slideEl = params.renderSlide ? $(params.renderSlide.call(swiper, slide, index$$1)) : $('<div class="' + swiper.params.slideClass + '" data-swiper-slide-index="' + index$$1 + '">' + slide + '</div>');
...
var Parallax = {
  ...
  setTranslate: function setTranslate() {
    ...
    slides.each(function (slideIndex, slideEl) {
      ...
      $(slideEl).find('[data-swiper-parallax], [data-swiper-parallax-x], [data-swiper-parallax-y], [data-swiper-parallax-opacity], [data-swiper-parallax-scale]').each(function (index$$1, el) {
  ...
  load: function load() {
    ...
    if (swiper.params.watchSlidesVisibility) {
      $wrapperEl.children('.' + swiperParams.slideVisibleClass).each(function (elIndex, slideEl) {
        var index$$1 = isVirtual ? $(slideEl).attr('data-swiper-slide-index') : $(slideEl).index();
...
var History = {
  ...
  setHistory: function setHistory(key, index$$1) {
    ...
    if (swiper.params.history.replaceState) {
      win$1.history.replaceState({ value: value }, null, value);
    } else {
      win$1.history.pushState({ value: value }, null, value);
    }
  },
...
var LOGO_SELECTOR = '[data-carousel-logo]';

var integrationsCarousel = function () {
  /* eslint-disable no-unused-vars */
  var mySwiper = new Swiper(CAROUSEL_SELECTOR, {
    speed: 800,
    slidesPerView: 1,
    effect: 'fade',
    pagination: {
      ...
      slideChange: function slideChange() {
        ...
        for (var i = 0; i < slides.length; i++) {
          if (slides[i].classList.contains('swiper-slide-active')) {
            logo = slides[i].querySelector(LOGO_SELECTOR);
            logo.classList.add('carousel-item__logo--animated');
          } else {
            logo = slides[i].querySelector(LOGO_SELECTOR);
            logo.classList.remove('carousel-item__logo--animated');
          }
        }
      }
    }
...
var COMPONENT_SELECTOR = '[data-content]';
var PROGRESS_SELECTOR = '[data-progress]';

var readingProgress = function () {
  var component = document.querySelector(COMPONENT_SELECTOR);

  if (component) {
    var progress = document.querySelector(PROGRESS_SELECTOR);

    window.addEventListener('scroll', function (e) {
      var scroll = window.pageYOffset !== undefined ? window.pageYOffset : (document.documentElement || document.body.parentNode || document.body).scrollTop;
      var position = scroll / (component.clientHeight - window.innerHeight) * 100;
...
var tabs = function () {
  ...
  for (var i = 0; i < tabs.length; i++) {
    tabs[i].addEventListener('click', function (e) {
      ...
      for (var _i2 = 0; _i2 < tabContent.length; _i2++) {
        tabContent[_i2].classList.remove('current');
      }

      document.querySelector('#' + tabId).classList.add('current');

      // add anchor to the tab link
      location.hash = tabId;
      e.preventDefault();
      window.scrollTo(0, 0);
  ...
  if (url.match('#')) {
    for (var _i3 = 0; _i3 < tabContent.length; _i3++) {
      tabContent[_i3].classList.remove('current');
    }
    document.querySelector('#' + url.split('#')[1]).classList.add('current');
    for (var _i4 = 0; _i4 < tabs.length; _i4++) {
      if (tabs[_i4].getAttribute('data-tab') === url.split('#')[1]) {
        tabs[_i4].classList.add('current');
      } else {
        tabs[_i4].classList.remove('current');
      }
    }
